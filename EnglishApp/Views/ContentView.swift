...
struct ContentView: View {
    ...
    var permanentCount: Int { permanentWords.count }
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 20) {
                ...
                // Permanent List
                List {
                    Section(header: Text("Permanent Vocabulary List (\(filteredPermanentWords.count))")) {
                        if permanentCount == 0 {
                            Text("No words mastered yet. Type translations during review to add them here!")
                                .font(.subheadline)
                                .foregroundColor(.secondary)
                                .italic()
                        } else if filteredPermanentWords.isEmpty {
    ...
    private func deleteWords(at offsets: IndexSet) {
        for index in offsets {
            let word = filteredPermanentWords[index]
